...
instance : ToString Round :=
  ⟨fun r => s!"[R:{r Color.red}, G:{r Color.green}, B:{r Color.blue}]"⟩

def Round.powerOf (rnd : Round) : Nat :=
  (rnd Color.red) * (rnd Color.green) * (rnd Color.blue)
...
/-- Minimal bag that would have made the  game possible. -/
def Game.minimalPossible (game : Game) : Round :=
  let maxReds := optNum (game.rounds.map (fun rnd => rnd Color.red)).maximum?
  let maxGreens := optNum (game.rounds.map (fun rnd => rnd Color.green)).maximum?
  let maxBlues := optNum (game.rounds.map (fun rnd => rnd Color.blue)).maximum?
  Round.mkRGB maxReds maxGreens maxBlues
...
def readGame (dat : String) : Game := by
  let dat' := dat.splitOn ": "
  exact { gid := optNum (readGameId? (optString dat'[0]?)),
...
def main : IO Unit := do
  ...
  let sumOfPowers := (Round.powerOf <$> Game.minimalPossible <$> games).sum
  IO.println s!"Answer 2 (sum of powers of minimal bags) : {sumOfPowers}"
